...
datos de la cuantificación de kallisto.
...
saveDb(x = txdb, file = "gencode.v43.chr_patch_hapl_scaff.annotation.gtf.gz")
...
## Conteos de los primeros transcriptomas
...
Antes de visualizar los resultados y hacer el análisis de expresión diferencial,
es necesario normalizar. El método que usa DESEq2 para normalizar es apto para
continuar con al DE analysis.

Para eso, primero hay que crear un objeto tipo data set de DESeq2. DESeq2 
necesita saber el tratamiento caso/control de cada muestra. Para saber eso,
descargamos la tabla de metadatos disponible en 
...
Una vez que tenemos nuestro DESeqDataSet, hacemos un pre-filtrado con conteos
bajos.
...
```{r}
library(apeglm)
# resLFC = lfcShrink(dds, coef = "disease_state_Covid.19_vs_normal", type = "apeglm")
plotMA(res)
```
